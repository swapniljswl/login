...
class Subsuperrole extends CI_Controller {
	...
	public function index()
	{
    	...
    	$this->load->view('login1/adminrole/subsuperrole',$data);
...
else
{
	...
	public function role($login_id)
	{
	 ...
	 $this->load->model('adminrole/Superrolemodel');
	if( $this->Superrolemodel->subrole($login_id)) 	
  		  {
		    $this->load->library('session');
			$this->session->set_flashdata('role','Sub Super admin role assigned .');
...
public function deleterole($login_id)
	{
	 
	 $this->load->model('adminrole/Superrolemodel');
	if( $this->Superrolemodel->deletesubrole($login_id)) 	
  		  {
		    $this->load->library('session');
			$this->session->set_flashdata('role','Sub Super admin role delete successfully .');
			$this->load->model('adminrole/Userrolemodel');
            $data['record1'] = $this->Userrolemodel->getdirctRecords();
			$this->load->model('login/Loginmodel');
	        $umenu['menu'] = $this->Loginmodel->menumaster();
