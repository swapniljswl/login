...
class Dirhome extends CI_Controller {
	...
	{	 
		$this->load->model('admin/Modelallletter');
        $data['sum']=$this->Modelallletter->sum();
		$data['prosum']=$this->Modelallletter->prosum();
		$data['levsum']=$this->Modelallletter->leavsum();
		$data['toursum']=$this->Modelallletter->toursum();
		$data['comapp']=$this->Modelallletter->comapp();
		
		$this->load->view('login1/login/dirhome.php',$data);	
